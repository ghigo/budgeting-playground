...
function runMigrations() {
  ...
  const categoryRulesInfo = db.prepare("PRAGMA table_info(category_rules)").all();
  const hasConfidenceOverride = categoryRulesInfo.some(col => col.name === 'confidence_override');
  const hasAutoGenerated = categoryRulesInfo.some(col => col.name === 'auto_generated');
  const hasRuleSource = categoryRulesInfo.some(col => col.name === 'rule_source');
  const hasCreatedAt = categoryRulesInfo.some(col => col.name === 'created_at');
  const hasUpdatedAt = categoryRulesInfo.some(col => col.name === 'updated_at');
